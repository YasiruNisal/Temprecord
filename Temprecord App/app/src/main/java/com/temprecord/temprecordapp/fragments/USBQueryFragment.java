package com.temprecord.temprecordapp.fragments;
...
import com.temprecord.temprecordapp.activities.GraphAcivity;
import com.temprecord.temprecordapp.activities.MainActivity;
import com.temprecord.temprecordapp.comms.BaseCMD;
import com.temprecord.temprecordapp.comms.CHUserData;
import com.temprecord.temprecordapp.comms.CommsSerial;
import com.temprecord.temprecordapp.comms.MT2Msg_Read;
import com.temprecord.temprecordapp.CustomLibraries.Yasiru_Temp_Library;
import com.temprecord.temprecordapp.comms.MT2Msg_Write;
import com.temprecord.temprecordapp.comms.MT2Values;
import com.temprecord.temprecordapp.comms.USBFragmentI;
import com.temprecord.temprecordapp.services.InputFilterMinMax;
import com.temprecord.temprecordapp.services.Json_Data;
import com.temprecord.temprecordapp.services.PDF;
...
//        bleFragmentI.onBLERead();
        ...
        openfile.setOnClickListener(clicked);
...
//                                SetUI();
                    ...
                    if(baseCMD.LoopOverwrite && !baseCMD.isLoopOverwriteOverFlow){//if loop overwrite is enabled
                        ...
                        state = 25;
                        mt2Msg_read = new MT2Msg_Read(commsChar.MEM_VAL, 0, baseCMD.SamplePointer *2-1, 64, 3);
                    }
                    usbFragmentI.onUSBWrite(commsSerial.WriteUSBByte(mt2Msg_read.Read_into_writeByte(true)));
                }
                break;
            case 25://normal read
                if (mt2Msg_read.write_into_readByte(commsSerial.ReadUSBByte(in)))
                {
                    ...
                    else
                    {
                        ...
                        if(fromRead)
                            Toast.makeText(getActivity(), getString(R.string.Loggerreading), Toast.LENGTH_LONG).show();
                    }
                    if(fromRead) {
                        fromRead = false;
                        stopProgress();
            ...
            case 26:
                if (mt2Msg_read.write_into_readByte(commsSerial.ReadUSBByte(in))) {
                    ...
                    Log.i("TEST", "length 1 " + mt2Msg_read.memoryData.length + " " + ReadValues1.length);
                }
                usbFragmentI.onUSBWrite(commsSerial.WriteUSBByte(mt2Msg_read.Read_into_writeByte(false)));
            ...
            case 28:
                if (mt2Msg_read.write_into_readByte(commsSerial.ReadUSBByte(in))) {
                    ...
                    System.arraycopy(ReadValues1,0,combo,0,ReadValues1.length);
                    System.arraycopy(ReadValues2,0,combo,ReadValues1.length,ReadValues2.length);
                    MT2ValueIn(combo);Log.i("TEST", "length 2 " + mt2Msg_read.memoryData.length + " " + combo.length);
 ...
 * */
    private void openfileDialogue() {
        ...
        // Get the layout inflater
        LayoutInflater inflater = getActivity().getLayoutInflater();
        final View popupView = inflater.inflate(R.layout.savedfile_dialog, null);

        savedfiles = popupView.findViewById(R.id.savedfileslist);

        final ArrayList<String> FilesInFolder = QS.GetFiles(Environment.getExternalStorageDirectory().getAbsolutePath().toString()+"/Temprecord");
        List<HashMap<String, String>> aList = new ArrayList<HashMap<String, String>>();
        for(int i = 0; i < FilesInFolder.size(); i++){
            HashMap<String, String> hm = new HashMap<String, String>();
            hm.put("file_name", FilesInFolder.get(i));
            hm.put("image_file", Integer.toString(R.drawable.pdfg));
            aList.add(hm);
        }
        String[] from = {"image_file", "file_name"};
        int[] to = {R.id.image_file, R.id.file_name};

        SimpleAdapter simpleAdapter = new SimpleAdapter(getContext(), aList, R.layout.listitem_file, from, to);

        savedfiles.setAdapter(simpleAdapter);

        savedfiles.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
                // Clicking on items
                QS.Open_PDF_in_Chrome(getContext(),Environment.getExternalStorageDirectory().getAbsolutePath().toString()+"/Temprecord/"+FilesInFolder.get(position) );
            }
        });
        builder1.setView(popupView)
                .setTitle(getString(R.string.openfile));
